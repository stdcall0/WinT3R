...
class GlobalLocalDecoderBlock(nn.Module):
    ...
    def forward(self, x, y, xpos, ypos, shape, global_merging=None, patch_width=None, patch_height=None):

        B, S, P, C = shape
        # print(f"x.shape: {x.shape}")    # f_img
        ...
        x = x + self.drop_path(self.attn(self.norm1(x), xpos, global_merging, patch_width, patch_height))
        x = x + self.drop_path(self.mlp(self.norm3(x)))
        ...
        x = x.reshape(B*S, P, C)
        xpos = xpos.reshape(B*S, P, -1)

        x = x + self.drop_path(self.attn_local(self.norm4(x), xpos))
...
if __name__ == "__main__":
    ...
    from croco.models_croco.pos_embed import get_2d_sincos_pos_embed, RoPE2D
    from functools import partial
